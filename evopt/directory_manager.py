...
Example:
	Basic usage to set up directories for an optimization run:
	
	>>> from evopt.directory_manager import DirectoryManager
	>>> dm = DirectoryManager("./optimization_results")
	>>> print(f"Run ID: {dm.dir_id}, Directory: {dm.evolve_dir}")
	>>> solution_dir = dm.create_solution_folder(epoch=0, solution=3)
"""
